...
if (!approve) { await aRef.update({ status:'denied', approvedByUserId: ctx.uid, approvedAt:new Date().toISOString(), decisionNote: note||null }); return { ok:true, status:'denied' }; }
