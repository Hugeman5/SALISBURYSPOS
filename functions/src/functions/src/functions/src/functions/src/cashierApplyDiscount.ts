...
export const cashierApplyDiscount = onCallDisc<{ orderId: string; lineId: string; discountType: "percent"|"amount"|"comp"|"promo"|"bogo"; discountValue: number; reason?: string }>(async (req) => {
  ...
  let pct=0; let discountCents=0;
  if (discountType==="percent") {
    pct = discountValue; discountCents = Math.round((ext*pct)/100);
  } else if (discountType==="amount") {
    discountCents = Math.round(discountValue); pct = ext? Math.round((discountCents/ext)*100):0;
  } else if (discountType==="comp") {
    discountCents = ext; pct = 100;
  } else {
    discountCents = Math.round(discountValue); pct = ext? Math.round((discountCents/ext)*100):0;
  }
