import { onCall as onCallMoveTab } from 'firebase-functions/v2/https';
import { getFirestore as getDbMoveTab } from 'firebase-admin/firestore';


export const moveTabToTable = onCallMoveTab<{ fromTableId: string; toTableId: string }>(async (req)=>{
...
const { fromTableId, toTableId } = req.data || {} as any; const db = getDbMoveTab();
const fromRef = db.collection('table_state').doc(fromTableId); const toRef = db.collection('table_state').doc(toTableId);
await db.runTransaction(async (tx)=>{
const [from, to] = await Promise.all([tx.get(fromRef), tx.get(toRef)]);
if (!from.exists) throw new Error('FROM_NOT_FOUND'); const orderId = from.get('orderId'); if (!orderId) throw new Error('NO_ORDER');
